...
function getResInfo(res) {
    ...
    rawHeaders.forEach((val, key) => {
        if (key === 'access-control-allow-origin' || key === 'access-control-expose-headers') {
            return;
        }
        if (key === '--s') {
            status = +val;
...
function isSimpleReqHdr(key, val) {
    ...
    if (key === 'accept' ||
        ...
        // 标准是总和小于 1024，这里保守一些
        return val.length < 256 && !R_UNSAFE_REQ_HDR_CHAR.test(val);
    }
}
...
export async function launch(req, urlObj, cliUrlObj) {
    ...
    while (method === 'GET') {
        const r = await getUrlCache(url);       /* 该资源是否加载过？ */
        if (r && r.host) {
        ...
        if (cdn.isDirectHost(urlObj.host)) {    /* 支持 CORS 的站点，可直连 */
        ...
        const ver = cdn.getFileVer(urlHash);    /* 常用静态资源 CDN 加速 */
        if (ver >= 0) {
    ...
    let resHdr;         /** @type {Headers} */

    if (host) {         /* 如果缓存未命中产生请求，服务器不做节点切换 */
        level = 0;
    }
    ...
    for (let i = 0; i < MAX_RETRY; i++) {
        ...
        if (rawInfo) {      /* 即使未命中缓存，在请求“加速节点”时也能带上文件信息 */
        ...
        try {
            ...
            console.warn(`fetch [${proxyUrl}] fail:`, err);
            break;
        ...
        if (mIsAcehOld && resHdr.has('--t')) {      /* 检测浏览器是否支持 aceh: * */
            mIsAcehOld = false;
            delete reqMap['--aceh'];
        }

        if (resHdr.has('--switched')) {             /* 是否切换节点 */
            rawInfo = resHdr.get('--raw-info');
            level++;
            continue;
        }

        const resErr = resHdr.get('--error');       /* 目前只有加速节点会返回该信息 */
        if (resErr) {
    ...
    const {status, headers, cookieStrArr} = getResInfo(res);
    ...
    const refresh = headers.get('refresh');         /* 处理 HTTP 返回头的 refresh 字段, 例如：http://www.otsukare.info/2015/03/26/refresh-http-header */
    if (refresh) {
